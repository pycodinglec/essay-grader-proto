...
1a. 프롬프트 공개 → 인증 직후 OCR/에세이 분할/채점 프롬프트를 expander로 표시
2. 채점기준 검증  → xlsx(번호/채점기준/배점) 양식 확인
3. 파일 업로드    → 채점기준표 검증 후 활성화 → PDF/이미지/ZIP 수용 → 이미지 변환
4. OCR 자동 실행 → 파일 처리 즉시 실행 → Nano Banana Pro API → JSON{학번, 이름, 에세이텍스트} 추출 + 진행률 표시
5. 제출물 식별    → 다중 페이지 병합 → 표로 사용자 확인
6. 평가           → 3 LLM 동일 프롬프트 평가 → 모델별 결과 보존, 최고점 채택
7. 리포트 생성    → report.xlsx(11개 컬럼: 학번/이름/작품번호/에세이/모델별 점수·피드백/최종 점수) + 진행률 표시
...
- 합산 점수가 가장 높은 응답 1개를 `best`로 채택, 모든 모델 결과는 `by_model`에 보존
