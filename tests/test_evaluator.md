...
### `TestBuildEvaluationPrompt` (6개 테스트)
- 채점기준표 텍스트 포함 확인
- 에세이 텍스트 포함 확인
...
- `gemini-3-flash` 모델명 사용 확인
- 프롬프트를 contents 파라미터로 전달 확인
- API 응답 텍스트 반환 확인
- `config.GOOGLE_API_KEY` 사용 확인
- 모든 테스트에서 `src.evaluator.genai`를 mock 처리

### `TestCallOpenai` (4개 테스트)
- `gpt-5.2` 모델명 사용 확인
- user 메시지로 프롬프트 전달 확인
- API 응답 content 반환 확인
- `config.OPENAI_API_KEY` 사용 확인
- 모든 테스트에서 `src.evaluator.openai`를 mock 처리

### `TestCallAnthropic` (5개 테스트)
- `claude-sonnet-4-6-20250514` 모델명 사용 확인
- user 메시지로 프롬프트 전달 확인
- `max_tokens=4096` 설정 확인
- API 응답 텍스트 반환 확인
- `config.ANTHROPIC_API_KEY` 사용 확인
- 모든 테스트에서 `src.evaluator.anthropic`를 mock 처리
